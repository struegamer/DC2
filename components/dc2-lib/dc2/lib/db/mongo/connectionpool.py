...
class ConnectionPool(object):
    ...
    def connect(self, name=None, host=None, port=None):
        if not self._get_instance_pid() in self._process:
            ...
            if name is None or name == "":
                name = "default"
            try:
                self._process[self._get_instance_pid()]["connection"] =\
                    Connection(host, port)
                self._process[self._get_instance_pid()]["name"] = name
                return True
            except:
                raise ConnectionError(
                    'Cannot connect to database on '
    ...
    def get_connection(self):
        if self._get_instance_pid() in self._process:
