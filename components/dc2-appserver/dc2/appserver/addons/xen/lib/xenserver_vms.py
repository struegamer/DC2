...
def xenserver_get_vms(xenhost=None, session_id=None, vmtype=None):
    ...
    if len(vmlist) > 0:
        for vm in vmlist:
            if (not parse_output(
                    s.VM.get_is_control_domain(
                        session_id, vm)) and
                    parse_output(s.VM.get_is_a_template(session_id, vm)) and
                    (vmtype == "template" or vmtype == "both")):
                vmlist_dict.append({
                    "session_id": session_id,
                    "xen_host": xenhost,
                    "vm_name": parse_output(
                        s.VM.get_name_label(session_id, vm)),
                    "vm_description": parse_output(
                        s.VM.get_name_description(session_id, vm)),
                    "vm_id": vm})
            if (not parse_output(
                s.VM.get_is_control_domain(session_id, vm)) and
                not parse_output(s.VM.get_is_a_template(session_id, vm)) and
                (vmtype == "vms" or
                    vmtype == "both")):
                vmlist_dict.append({
                    "session_id": session_id,
