...
@rpcmethod(
    name="dc2.configuration.environments.list",
    returns={"list environment_recs": "List of type environment_records"},
    params={"dict environment_rec": "Enviornment_record"},
...
@rpcmethod(
    name="dc2.configuration.environments.find",
    returns={"list environment_recs": "List of type environment_records"},
    params={"dict environment_rec": "Enviornment_record"},
    is_xmlrpc=True, is_jsonrpc=True)
...
@rpcmethod(
    name="dc2.configuration.environments.add",
    returns={"string doc_id": "Document ID"},
    params={"dict environment_rec": "Dictionary of type ENVIRONMENT_RECORDS"},
    is_xmlrpc=True, is_jsonrpc=True)
...
@rpcmethod(
    name="dc2.configuration.environments.update",
    returns={"string doc_id": "Document ID"},
    params={"dict environment_rec": "Dictionary of type ENVIRONMENT_RECORDS"},
    is_xmlrpc=True, is_jsonrpc=True)
def dc2_deployment_environments_update(env_rec=None):
    if env_rec is not None and type(env_rec) is types.DictType:
        if (check_record(env_rec, ENVIRONMENT_RECORDS) and
            '_id' in env_rec and
            tbl_environments.find_one(
                {"_id": env_rec["_id"]}) is not None):
            doc_id = tbl_environments.save(env_rec)
            return doc_id
...
@rpcmethod(
    name="dc2.configuration.environments.delete",
    returns={"bool success": "Document ID"},
    params={"dict env_rec": "environment record"},
    is_xmlrpc=True, is_jsonrpc=True)
...
@rpcmethod(
    name="dc2.configuration.environments.copy",
