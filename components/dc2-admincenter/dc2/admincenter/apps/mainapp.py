...
try:
    ...
    print('Error: {0} in File: {1}'.format(e, __file__))
    sys.exit(1)
...
class MainAppHandler(RequestHandler):
    ...
    def _import_controllers(self):
        ...
        for mod in CONTROLLER_MAPPINGS.keys():
            temp_import_path = 'dc2.admincenter.apps.controllers.{0}'.format(
                CONTROLLER_MAPPINGS[mod])
