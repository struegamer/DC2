...
tbl_systemusers = Table(
    MONGOS["dc2db"]["database"].get_table("defaultsystemusers"))
...
@rpcmethod(
    name="dc2.configuration.systemusers.list",
...
@rpcmethod(
    name="dc2.configuration.systemusers.add",
...
def dc2_configuration_systemusers_add(rec=None):
    if rec is not None and type(rec) is types.DictType:
        if (check_record(rec, SYSUSERS_RECORD) and
            'username' in rec and
                tbl_systemusers.find_one(
                    {'username': rec["username"]}) is None):
...
@rpcmethod(
    name="dc2.configuration.systemusers.update",
    params={}, returns={}, is_xmlrpc=True, is_jsonrpc=True)
def dc2_configuration_systemusers_update(rec=None):
    if rec is not None and type(rec) is types.DictType:
        if (check_record(rec, SYSUSERS_RECORD) and
            'username' in rec and
                tbl_systemusers.find_one(
                    {'username': rec["username"]}) is not None):
            doc_id = tbl_systemusers.save(rec)
            return doc_id
...
@rpcmethod(
    name="dc2.configuration.systemusers.delete",
    params={}, returns={}, is_xmlrpc=True, is_jsonrpc=True)
