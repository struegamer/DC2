...
tbl_installstate=Table(MONGOS["dc2db"]["database"].get_table("installstatus"))
...
@rpcmethod(name="dc2.inventory.hosts.list",params={},returns={},is_xmlrpc=True,is_jsonrpc=True)
...
@rpcmethod(name="dc2.inventory.hosts.find",params={},returns={},is_xmlrpc=True,is_jsonrpc=True)
def dc2_deployment_hosts_find(search=None):
    return dc2_deployment_hosts_list(search)


@rpcmethod(name="dc2.inventory.hosts.get",params={},returns={},is_xmlrpc=True,is_jsonrpc=True)
...
@rpcmethod(name="dc2.inventory.hosts.add",params={},returns={},is_xmlrpc=True,is_jsonrpc=True)
def dc2_deployment_hosts_add(record=None):
    if record is not None and type(record) is types.DictType:
        if check_record(record,HOST_RECORD) and tbl_host.find_one({"server_id":record["server_id"]}) is None:
            doc_id=tbl_host.save(record)
            return doc_id
    return xmlrpclib.Fault(-32501, "Record couldn't be added")
        
@rpcmethod(name="dc2.inventory.hosts.update",params={},returns={},is_xmlrpc=True,is_jsonrpc=True)
def dc2_deployment_hosts_update(record=None):
    if record is not None and type(record) is types.DictType:
        if check_record(record,HOST_RECORD):
            ...
            if old_host_record is not None and old_host_record["server_id"]==record["server_id"]:
                doc_id=tbl_host.save(record)
                install_rec=tbl_installstate.find_one({"server_id":record["server_id"]})
                if install_rec["hostname"]!="%s.%s" % (record["hostname"],record["domainname"]):
                    install_rec["hostname"]="%s.%s" % (record["hostname"],record["domainname"])
                    tbl_installstate.save(install_rec)                    
            else:
                #
                # Check for double host server id
                #
                if tbl_host.find_one({"server_id":record["server_id"]}) is not None:
                    return xmlrpclib.Fault(-32501,"The Server already has a host attached.")
                doc_id=tbl_host.save(record)
                install_rec=tbl_installstate.find_one({"host_id":record["_id"]})
...
@rpcmethod(name="dc2.inventory.hosts.remove",params={},returns={},is_xmlrpc=True,is_jsonrpc=True)
def dc2_inventory_hosts_remove(record=None):
    if record is not None and type(record) is types.DictType:
        if record.has_key("_id") or record.has_key("server_id"):
