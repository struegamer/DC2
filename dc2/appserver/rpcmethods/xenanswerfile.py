...
tbl_xenanswerfiles = Table(MONGOS["dc2db"]["database"].get_table(
    "xen_answerfile"))
...
@rpcmethod(
    name="dc2.configuration.xen.answerfile.list",
...
@rpcmethod(
    name="dc2.configuration.xen.answerfile.add",
    params={}, returns={}, is_xmlrpc=True, is_jsonrpc=True)
def dc2_configuration_xenanswerfile_add(rec=None):
    if rec is not None and type(rec) is types.DictType:
        if (check_record(rec, XEN_RECORD) and
            'filename' in rec and
            tbl_xenanswerfiles.find_one(
                {"filename": rec["filename"]}) is None):
...
@rpcmethod(
    name="dc2.configuration.xen.answerfile.update",
    params={}, returns={}, is_xmlrpc=True, is_jsonrpc=True)
...
@rpcmethod(
    name="dc2.configuration.xen.answerfile.delete",
    params={}, returns={}, is_xmlrpc=True, is_jsonrpc=True)
def dc2_configuration_xenanswerfiles_delete(rec=None):
    if rec is not None and type(rec) is types.DictType:
        if '_id' in rec:
            response = tbl_xenanswerfiles.remove(rec)
...
@rpcmethod(
    name="dc2.configuration.xen.answerfile.get",
    params={}, returns={}, is_xmlrpc=True, is_jsonrpc=True)
def dc2_configuration_xenanswerfiles_get(rec=None):
    if rec is not None and type(rec) is types.DictType:
        if 'filename' in rec or '_id' in rec:
