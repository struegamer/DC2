...
@rpcmethod(
    name="dc2.deployment.installstate.find",
...
@rpcmethod(
    name='dc2.deployment.installstate.list',
    params={}, returns={}, is_xmlrpc=True, is_jsonrpc=True)
...
@rpcmethod(
    name="dc2.deployment.installstate.add",
    params={}, returns={}, is_xmlrpc=True, is_jsonrpc=True)
...
@rpcmethod(
    name="dc2.deployment.installstate.update",
    params={}, returns={}, is_xmlrpc=True, is_jsonrpc=True)
def dc2_deployment_installstate_update(record=None):
    if record is not None and type(record) is types.DictType:
        if (check_record(record, INSTALLSTATUS_RECORD) and
                tbl_installstatus.find_one(
                    {"server_id": record["server_id"],
                        "host_id": record["host_id"]}) is not None):
            doc_id = tbl_installstatus.save(record)
            return doc_id
...
@rpcmethod(
    name="dc2.deployment.installstate.remove",
    params={}, returns={}, is_xmlrpc=True, is_jsonrpc=True)
