...
def dc2_configuration_classtemplates_add(record=None):
    if record is not None and type(record) is types.DictType:
        if check_record(record, TEMPLATE_RECORD) and record.has_key("name") and tbl_templates.find_one({"name":record["name"]}) is None:
            try:
                doc_id = tbl_templates.save(record)
                return doc_id
            except Exception,e:
                return xmlrpclib.Fault(-32501, "Record couldn't be added %s " % e)
    return xmlrpclib.Fault(-32501, "Record couldn't be added")

@rpcmethod(name="dc2.configuration.classtemplates.update",params={},returns={},is_xmlrpc=True,is_jsonrpc=True)
def dc2_configuration_classtemplates_update(record=None):
    if record is not None and type(record) is types.DictType:
        if check_record(record, TEMPLATE_RECORD) and record.has_key("name") and record.has_key("_id") and tbl_templates.find_one({"name":record["name"]}) is not None:
            try:
                doc_id = tbl_templates.save(record)
                return doc_id
            except Exception,e:
...
@rpcmethod(name="dc2.configuration.classtemplates.get",params={},returns={},is_xmlrpc=True,is_jsonrpc=True)
...
@rpcmethod(name="dc2.configuration.classtemplates.remove",params={},returns={},is_xmlrpc=True,is_jsonrpc=True)
