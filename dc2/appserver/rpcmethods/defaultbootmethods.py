...
tbl_bootmethods=Table(MONGOS["dc2db"]["database"].get_table("default_bootmethods"))
...
@rpcmethod(name="dc2.configuration.bootmethods.list",params={},returns={},is_xmlrpc=True,is_jsonrpc=True)
...
@rpcmethod(name="dc2.configuration.bootmethods.add",params={},returns={},is_xmlrpc=True,is_jsonrpc=True)
def dc2_configuration_defaultbootmethods_add(rec=None):
    if rec is not None and type(rec) is types.DictType:
        if check_record(rec,DEFBOOT_RECORD) and rec.has_key("hardware_type") and tbl_bootmethods.find_one({"hardware_type":rec["hardware_type"]}) is None:
            doc_id = tbl_bootmethods.save(rec)
            return doc_id
    return xmlrpclib.Fault(-32501,"Record could not be added")

@rpcmethod(name="dc2.configuration.bootmethods.update",params={},returns={},is_xmlrpc=True,is_jsonrpc=True)
def dc2_configuration_defaultbootmethods_update(rec=None):
    if rec is not None and type(rec) is types.DictType:
        if check_record(rec,DEFBOOT_RECORD) and rec.has_key("hardware_type") and tbl_bootmethods.find_one({"hardware_type":rec["hardware_type"]}) is not None:
            doc_id=tbl_bootmethods.save(rec)
            return doc_id
    return xmlrpclib.Fault(-32501,"Record could not be updates")

@rpcmethod(name="dc2.configuration.bootmethods.delete",params={},returns={},is_xmlrpc=True,is_jsonrpc=True)
...
@rpcmethod(name="dc2.configuration.bootmethods.update_hw_types",params={},returns={},is_xmlrpc=True,is_jsonrpc=True)
def dc2_configuration_defaultbootmethods_update_hardware_types():
    server_list=[]
    server_list=tbl_servers.find()
    for server in server_list:
        if server.has_key('product_name') and server["product_name"] is not None and server["product_name"] != "":
            if tbl_bootmethods.find_one({"hardware_type":server["product_name"]}) is None:
                dc2_configuration_defaultbootmethods_add({"hardware_type":server["product_name"],"pxe_bootmethod":"none"})
    return True
