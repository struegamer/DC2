...
@rpcmethod(name="dc2.inventory.servers.macaddr.list", returns={"list mac_addr_rec":"List of MAC Address Records for a server"}, params={}, is_xmlrpc=True, is_jsonrpc=True)
...
def dc2_servers_macaddr_add(rec_mac=None):
    if rec_mac is not None and type(rec_mac) is types.DictType:
        if check_record(rec_mac, MACADDR_RECORD) and tbl_server.find_one({"mac_addr":rec_mac["mac_addr"]}) is None:
...
def dc2_servers_macaddr_update(rec_mac=None):
    if rec_mac is not None and type(rec_mac) is types.DictType:
        if check_record(rec_mac, MACADDR_RECORD) and tbl_server.find_one({"_id":rec_mac["_id"], "server_id":rec_mac["server_id"]}) is not None:
            doc_id = tbl_server.save(rec_mac)
            return doc_id
    return xmlrpclib.Fault(-32504, "Record couldn't be updated")

@rpcmethod(name="dc2.inventory.servers.macaddr.delete", returns={"bool success":"True if action was successful"}, params={"dict rec_macaddr":"Prefilled record dictionary with key _id, or server_id to delete all mac_addr records attached to a server"}, is_xmlrpc=True, is_jsonrpc=True)
def dc2_servers_macaddr_delete(rec_mac=None):
    if rec_mac is not None and type(rec_mac) is types.DictType:
        if rec_mac.has_key("_id") or rec_mac.has_key("server_id"):
...
@rpcmethod(name="dc2.inventory.servers.macaddr.find", returns={"bool success":"True if action was successful"}, params={"dict rec_server":"Pre-Filled record dictionary with key _id"}, is_xmlrpc=True, is_jsonrpc=True)
