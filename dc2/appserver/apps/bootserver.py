...
class BootServer(object):
    ...
    def GET(self, name):
        ...
        if found is not None and len(found.groups()) > 0:
            ...
            mac_list = s.dc2.inventory.servers.macaddr.find(
                {"mac_addr": mac_addr})
            if len(mac_list) > 0 and mac_list[0] is not None:
                mac_rec = mac_list[0]
                host_list = s.dc2.inventory.hosts.list(
                    {"server_id": mac_rec["server_id"]})
                if host_list is not None and len(host_list) > 0:
                    host_rec = host_list[0]
                    installstate = s.dc2.deployment.installstate.get(
                        {"host_id": host_rec["_id"]})
                    if 'environments' in host_rec:
                        environ_rec = s.dc2.configuration.environments.list(
                            {"name": host_rec["environments"]})[0]
                        env_variables = self.convert_to_dict(
                            environ_rec["variables"])
                    else:
                        pass
                    if installstate is not None:
                        if ('status' in installstate and
                                installstate["status"] == "localboot"):
                            servers = s.dc2.inventory.servers.find(
                                {"_id": host_rec["server_id"]})
                            server = None
                            if servers is not None and len(servers) > 0:
                                server = servers[0]
                            if (server is not None and
                                    'product_name' in server and
                                    server["product_name"] is not None and
                                    server["product_name"] != ""):
                                bootmethods = \
                                    s.dc2.configuration.bootmethods.list(
                                        {"hardware_type":
                                            server["product_name"]})
                                bootmethod = None
                                if (bootmethods is not None and
                                        len(bootmethods) > 0):
                                    bootmethod = bootmethods[
                                        0]["pxe_bootmethod"]
                                return self.write_pxefile(
                                    mac_addr,
                                    "localboot", bootmethod=bootmethod)
                            else:
                                return self.write_pxefile(
                                    mac_addr, "localboot")
                        if ('status' in installstate and
                                installstate["status"] == "deploy"):
                            result = self.write_pxefile(
                                mac_addr, 'deploy', env_variables)
                            if result is None:
                                raise web.notfound()
                            return result
                        if ('status' in installstate and
                                installstate["status"] == "xenserver"):
                            pass
            else:
                environ_rec = s.dc2.configuration.environments.list(
                    {"name": "INVENTORY"})[0]
                ...
                result = self.write_pxefile(
                    mac_addr, 'inventory', env_variables)
                if result is None:
                    raise web.notfound()
                return result
        else:
    ...
    # def write_pxefile(self, filename, action=None, kernel_name=None,
    # initrd_name=None, nfs_root=None, backend_url=None, bootmethod=None):
    def write_pxefile(
        self,
        filename,
        action=None,
