    ...
    def _update_status(self,installstate_record=None,status=None,progress=None):
        if installstate_record is not None:
            ...
            if status is not None or progress is not None:
                self._proxy.dc2.deployment.installstate.update(installstate_record)
            return True
        return False
    def find_status_by_server(self,serial_no=None):
        if serial_no is not None and serial_no != "":
            ...
            if server_rec is not None:
                installstate=self._proxy.dc2.deployment.installstate.get({"server_id":server_rec["_id"]})
    ...
    def find_status_by_host(self,hostname=None,domainname=None):
        if hostname is not None and hostname != "" and domainname is not None and domainname != "":
            ...
            if host_rec is not None:
                installstate=self._proxy.dc2.deployment.installstate.get({"host_id":host_rec["_id"]})
                if installstate is not None:
                    return installstate
        return None
